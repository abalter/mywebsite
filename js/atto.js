    ...
    updatePage(query_obj) {
        ...
        if ('source' in query_obj && 'target' in query_obj) {
            ...
            return $.get(source, function (markdown, status) {
                ...
                // if this.plugins is not empty, then run the scripts listed in this.plugins
                if ($self.plugins && $self.plugins.length > 0) {
                    debug("Loading plugins: " + $self.plugins.join(", "), 1);
                    $self.plugins.forEach(plugin => {
                        let pluginPath = `plugins/${plugin}/${plugin}.js`;
                        debug("Loading plugin: " + pluginPath, 1);
                        $.getScript(pluginPath)
                            .done(() => debug("Successfully loaded plugin: " + plugin, 1))
                            .fail((jqxhr, settings, exception) => debug("Failed to load plugin " + plugin + ": " + exception, 1));
                    });
                }
